...
(x_train, y_train), (x_test, y_test) = keras.datasets.mnist.load_data(path="mnist.npz") #5 points
...
x_train = x_train / 255 #5 points
x_test = x_test / 255 #5 points
...
model.add(keras.layers.Dense(10, activation=keras.activations.softmax)) #5 points

# Explain why the softmax activation function was used for the output layer.
print("The softmax action function was use for the output layer because it will convert the output layer into a probablity distribution. In other words each output node will have value from 0-1 but the sum of the output layer will equal 1") #5 points 
...
print("The size of the first hidden layer is 300. None means the batch size, specifically variable batch sizes.\
The total number of parameters of the first hidden layer is 235500, which refers to the number of connections is has to the previous layer") #8 points
model.summary()
...
print("An epoch is an iternation of the dataset. Or one 'step' of using the optimizer to tweak the weight of the connection with respect to how it perform on the training dataset.") #6 points 
